...
pdf_load_image_imp(fz_context *ctx, pdf_document *doc, pdf_obj *rdb, pdf_obj *dict, fz_stream *cstm, int forcemask)
{
	...
	fz_try(ctx)
	{
		...
		if (cstm == NULL)
		{
             ...
             *-----------------------------------*/
            int num = 0;
            if( pdf_is_indirect(ctx, dict) ) num = pdf_to_num(ctx, dict);
            else num = pdf_obj_parent_num(ctx, dict);

            if( 0!=num ) {
			/* Just load the compressed image data now and we can decode it on demand. */
                // modified by zl
                // change pdf_to_num(ctx, dict) to num
                // or may cause crush!!!!
                buffer = pdf_load_compressed_stream(ctx, doc, num);
...
pdf_add_image(fz_context *ctx, pdf_document *doc, fz_image *image, int mask)
{
    ...
    if(imref) {
        // fixed by zl03jsj:
        // the founded imref may not be a ref obj
                ...
                while (h--)
                {
                    ...
                    while (size2--)
                    {
                        *d++ = *s++;
                        ...
                        if (mod == 0) {
                            // modified by zl
                            // if alpha channel is 0, set color to white
                            if( !(*s) ){
                                mod = n;

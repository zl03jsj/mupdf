...
JNIEXPORT void JNICALL
JNI_FN(MuPDFCore_addInkAnnotationInternal)(JNIEnv * env, jobject thiz, jobjectArray arcs, jstring password, float inkThickness, int inkColor)
{
	globals *glo = get_globals(env, thiz);
	fz_context *ctx = glo->ctx;
	fz_document *doc = glo->doc;
	...
	fz_try(ctx)
	{
        ...
        pdf_dict_puts_drop(ctx, annot->obj, ANNOT_PASSWORD_NAME, 
	...
	int w, int h, jbyteArray imgdata)
{
	...
	signed char *data = (*env)->GetByteArrayElements(env, imgdata, NULL);
	fz_buffer *imgbuff = fz_new_buffer_from_shared_data(ctx, (const char*)data, size);
	...
	fz_try(ctx) {
		...
		int okay = pdf_add_image_with_document(ctx, doc, imgbuff, pageno, x, y, w, h);
    ...
    fz_always(ctx) {
        if(data) (*env)->ReleaseByteArrayElements(env, imgdata, data, 0);
